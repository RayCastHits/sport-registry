  ...
  $.fn.tablemanager = function(options = null) {
    ...
    if (options !== null) {
      ...
      if (pagination === true || Table.hasClass('tablePagination')) {
        ...
        var pagesDiv = '<ul  id="pagination" class="pagination"></ul>'
      ...
      if (options.appendFilterby === true || Table.hasClass('tableFilterBy')) {
        ...
        var filterbyDiv =
          ...
          ': </label><select id="filter_by"></select> <input id="filter_input" class="form-control w-10" type="text" placeholder="' +
    ...
    function multipleSortCol(array, data) {
      ...
      array.sort(function(a, b) {
        for (i = 0; i < cols.length; i++) {
          ...
          if (i == cols.length && a[cols[i]] == b[cols[i]]) {
            return 0
          }
          ...
          if (types[i] == 'alphanumeric') {
            ...
            if (
              formatDate(formats[i], a[cols[i]]) >
              formatDate(formats[i], b[cols[i]])
            ) {
              return order
            } else if (
              formatDate(formats[i], a[cols[i]]) <
    ...
    function appendPageControllers() {
      ...
      $('#pagination').append(
        $('<button>', {
          value: 'first',
          text: '<<',
          class: 'btn btn-primary btn-sm pagecontroller pagecontroller-f'
        })
      )
      // Previous
      $('#pagination').append(
        $('<button>', {
          value: 'prev',
          text: '<',
          class: 'btn btn-primary btn-sm  pagecontroller pagecontroller-p'
        })
      )
      // Numbers
      for (i = 1; i <= numPages; i++) {
        $('#pagination').append(
          $('<button>', {
            value: i,
            text: i,
            class: 'btn btn-primary btn-sm pagecontroller pagecontroller-num'
          })
        )
      }
      // Next
      $('#pagination').append(
        $('<button>', {
          value: 'next',
          text: '>',
          class: 'btn btn-primary btn-sm pagecontroller pagecontroller-n'
        })
      )
      // Last
      $('#pagination').append(
        $('<button>', {
          value: 'last',
          text: '>>',
          class: 'btn btn-primary btn-sm pagecontroller pagecontroller-l'
        })
      )
    }
